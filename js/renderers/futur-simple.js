(function initializeSimpleTenseRenderers(global) {
  "use strict";

  const FR = global.FR = global.FR || {};
  FR.renderers = FR.renderers || {};

  function createRenderer(key, slug, tenseTitle) {
    const tenseLabel = tenseTitle.toLocaleLowerCase("fr");
    ...
    function reportAudioError(button, error) {
      ...
      if (error) console.error(`${tenseTitle} audio failed`, error);
    }

    function playForm(text, button) {
      if (typeof global.speakSequence !== "function") {
        console.warn(`Global speakSequence is not available for ${tenseLabel} audio.`);
    ...
    function createSingleFormMatrix(item) {
      ...
      matrix.setAttribute("aria-label", `${item.label}: ${tenseLabel} conjugation`);
    ...
    function createMatrix(item, data) {
      ...
      matrix.className = "verb-matrix";
      matrix.setAttribute("role", "table");
      matrix.setAttribute("aria-label", `${item.label}: ${tenseLabel} conjugations`);
    ...
    function createExampleButton(sentence, type, item) {
      ...
      button.type = "button";
      button.setAttribute("aria-label", `Play ${type.label.toLowerCase()} ${tenseLabel} example: ${example.fr}. English: ${example.en}`);
    ...
    function createExamplesSection(item, panelId) {
      ...
      title.textContent = `${tenseTitle} examples`;
    ...
    function createVerbCard(item, data) {
      if (!item || typeof item !== "object") throw new Error(`${tenseTitle} item is malformed.`);
      if (item.error) throw item.error;
      if (!item.formula || !Array.isArray(item.rows) || !item.rows.length) {
        throw new Error(`${item.label} has no derived ${tenseLabel} paradigm.`);
      }

      const panel = document.createElement("div");
      panel.className = "panel verb-panel";
      panel.id = `${slug}-${slugify(item.key)}`;
      panel.tabIndex = -1;
    ...
    function createGroup(groupData, data, cardTargets, groupIndex, errors) {
      ...
      group.id = `${slug}-group-${slugify(groupData.key)}`;
      group.dataset[`${key}Group`] = groupData.key;
      ...
      FR.utils.deferRender(group, () => {
        groupData.items.forEach(item => {
          try {
            ...
            failure.id = `${slug}-${slugify(item.key)}`;
            failure.tabIndex = -1;
            cardTargets.get(item.key).card = failure;
            content.appendChild(failure);
            console.error(`${tenseTitle} card failed to render`, item, error);
          }
        });
      });
    ...
    function renderGroups(container, data) {
      ...
      if (data.errors.length) {
        data.errors.forEach(entry => {
          ...
          container.appendChild(createErrorCard(`${entry.label || tenseTitle} failed to load`, error));
        });
      }
      ...
      data.groups.forEach((group, index) => {
        try {
          ...
          console.error(`${tenseTitle} group failed to render`, group, error);
        }
      });
      return { cardTargets, errors };
    }
    ...
    function renderIndex(container, data, cardTargets) {
      ...
      container.setAttribute("aria-label", `${tenseTitle} index`);
    ...
    function render() {
      const data = FR.data && FR.data[key];
      const specialRules = document.getElementById(`${key}SpecialRules`);
      const index = document.getElementById(`${key}Index`);
      const groupStack = document.getElementById(`${key}GroupStack`);
      const missingTargets = [];
      const errors = [];

      if (!specialRules) missingTargets.push(`#${key}SpecialRules`);
      if (!index) missingTargets.push(`#${key}Index`);
      if (!groupStack) missingTargets.push(`#${key}GroupStack`);
      if (!data) {
        const error = new Error(`FR.data.${key} is not available.`);
        [specialRules, index, groupStack].filter(Boolean).forEach(container => {
          container.replaceChildren(createErrorCard(`${tenseTitle} failed to initialize`, error));
        });
        console.error(`${tenseTitle} renderer could not find its data`, error);
      ...
      try {
        ...
        console.error(`${tenseTitle} rules failed to render`, error);
      }
      ...
      try {
        ...
        errors.push(error);
        if (groupStack) groupStack.replaceChildren(createErrorCard(`${tenseTitle} groups failed to render`, error));
        console.error(`${tenseTitle} groups failed to render`, error);
      }
      ...
      try {
        ...
        errors.push(error);
        if (index) index.replaceChildren(createErrorCard(`${tenseTitle} index failed to render`, error));
        console.error(`${tenseTitle} index failed to render`, error);
      }
    ...
    return { render, renderSpecialRules, renderIndex, renderGroups };
  }

  FR.renderers.futurSimple = createRenderer("futurSimple", "futur-simple", "Futur simple");
  FR.renderers.conditionnelPresent = createRenderer("conditionnelPresent", "conditionnel-present", "Conditionnel présent");
})(window);
